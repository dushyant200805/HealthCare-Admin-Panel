import { useEffect } from "react";

function EditDepartmentModal() {
  useEffect(() => {
    if ($("#edt_depart_detail").length > 0) {
      $("#edt_depart_detail").validate({
        rules: {
          edt_depart_name: "required",

          edt_depart_head: "required",

          edt_depart_status: "required",
        },

        messages: {
          edt_depart_name: "This field is required",
          edt_depart_head: "This field is required",
          edt_depart_date: "This field is required",
          edt_depart_status: "Please select status",
        },

        errorElement: "small",

        errorClass: "text-danger mt-1 d-block",

        errorPlacement: function (error, element) {
          if (element.closest(".input-group").length) {
            error.insertAfter(element.closest(".input-group"));
          } else if (element.attr("type") === "radio") {
            error.appendTo(element.closest(".custom-err"));
          } else {
            error.insertAfter(element);
          }
        },

        highlight: function (element) {
          $(element).addClass("border-danger");
        },

        unhighlight: function (element) {
          $(element).removeClass("border-danger");
        },
      });
    }
  }, []);
  return (
    <>
      <div
        className="modal fade"
        id="editdepartment"
        tabIndex="-1"
        aria-hidden="true"
      >
        <div className="modal-dialog  rounded-10 modal-dialog-centered">
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="text-dark text-capitalize mb-0 p-3 text-gray">
                Edit Department
              </h5>
              <button
                type="button"
                className="btn-close"
                data-bs-dismiss="modal"
                aria-label="Close"
              ></button>
            </div>
            <div className="modal-body">
              <div className="px-4  ">
                <form id="edt_depart_detail">
                  <div className="mb-3">
                    <label htmlFor="edt_depart_name" className="mb-2 text-dark">
                      Department Name
                      <sup>
                        <span className="required">*</span>
                      </sup>
                    </label>
                    <input
                      type="text"
                      className="form-control rounded "
                      id="edt_depart_name"
                      name="edt_depart_name"
                      defaultValue="Cardiology"
                    />
                  </div>
                  <div className="mb-3">
                    <label htmlFor="edt_depart_head" className="mb-2 text-dark">
                      Department Head
                      <sup>
                        <span className="required">*</span>
                      </sup>
                    </label>
                    <input
                      type="text"
                      className="form-control rounded "
                      id="edt_depart_head"
                      name="edt_depart_head"
                      defaultValue="Dr. Andrea Lalema"
                    />
                  </div>
                  <div className="mb-3">
                    <label htmlFor="edt_description" className="mb-2 text-dark">
                      Description
                    </label>
                    <textarea
                      name="edt_description"
                      id="edt_description"
                      cols="30"
                      rows="4"
                      className="form-control"
                      defaultValue="Lorem ipsum, dolor sit amet consectetur adipisicing elit. Fugit libero sequi provident? Molestias quas dolor vero quasi suscipit, delectus debitis"
                    />
                  </div>
                  <div className="mb-5  d-flex flex-column justify-content-center">
                    <label className="text-dark">
                      Status
                      <sup>
                        <span className="required">*</span>
                      </sup>
                    </label>
                    <div className="d-flex gap-2 align-items-center custom-err">
                      <span>
                        <input
                          type="radio"
                          name="edt_depart_status"
                          defaultChecked
                        />
                        <span className="ms-1">Active</span>
                      </span>
                      <span>
                        <input type="radio" name="edt_depart_status" />
                        <span className="ms-1">In Active</span>
                      </span>
                    </div>
                  </div>
                  <div className="text-end">
                    <button
                      type="reset"
                      className="btn btn-outline-dark me-2 mw-120 rounded-pill py-2"
                      data-bs-dismiss="modal"
                      aria-label="Close"
                    >
                      Cancel
                    </button>
                    <button
                      type="submit"
                      className="btn btn-primary rounded-4 fw-500 py-2 rounded-pill mw-120 px-4 text-capitalize text-white"
                    >
                      Submit
                    </button>
                  </div>
                </form>
              </div>
            </div>
          </div>
        </div>
      </div>
    </>
  );
}
export default EditDepartmentModal;
